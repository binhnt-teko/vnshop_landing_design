...
export default class PropsComp extends React.Component {
  ...
  render() {
    ...
    // console.log("binhnt.PropsComp: edit = ", edit);
    // console.log("binhnt.PropsComp: currentEditData = " , currentEditData);
    // console.log("binhnt.PropsComp: templateData = ", templateData);

    const editArray = edit ? edit.split(',').map((c) => c.trim()).filter((c) => noProps.indexOf(c) === -1) : [];
    if (!edit || !editArray.length) {
      return null;
    }
    console.log("binhnt.PropsComp: editArray = ", editArray);

    const { id } = currentEditData;
    const ids = id.split('-');
    const cid = ids[0].split('_')[0];

    // console.log("binhnt.PropsComp: ids = " , ids);
