...
import { mergeEditDataToDefault, isImg } from '../share/utils';
import tempData from '../config/template.list';
import otherComp from '../share/otherToString';
import lessComp from '../share/static/lessToString';
...
const setScrollScreen = () => {
  const str = `// 实现整屏滚动
    scrollScreen.init({ location: ['${templateStrObj.TEMPLATE.map((c) => !c.match(/Nav/ig) && c).filter((c) => c).join('\', \'')}'] });`;
...
export function saveJsZip(templateData, callBack, getJSON) {
  ...
  // Each save reset saves the target data. .
  templateStrObj = {
    JS: {},
    LESS: {},
    PROPS: {},
  ...
  template.forEach((key) => {
    ...
    console.log("keys[0] = ", JSON.stringify(keys[0]))

    const { templateStr, less } = tempData[keys[0]];
    const dataSource = mergeEditDataToDefault(config[key], tempData[keys[0]], true);
