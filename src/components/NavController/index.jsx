...
class NavController extends React.PureComponent {
  ...
  onPreview = () => {
    ...
    // If you clear the data on the preview page, and then generate the preview, there will be no data, manually write；
    ...
    // const url = `${location.port ? `${location.protocol}//${location.hostname}:7113`
    //   : `${location.origin}/`}#uid=${templateData.uid}`;
    const url = `${location.protocol}//${location.hostname}:${location.port}/templates/`;
  ...
  setRightSideMenuOpen = () => {
    console.log("binhnt.components.navcontroller: setRightSideMenuOpen: ");
    const { onRightMenuOpen } = this.props;
    if (onRightMenuOpen !== undefined) {
      onRightMenuOpen(!this.state.rightSideMenuOpen);
    }
    this.setState({
  ...
  render() {
    ...
    const menuChild = [
      ...
      // {
      //   name: <FormattedMessage id="app.header.test" key="m" />,
      //   icon: <Icon component={() => RemoveLocalStorage('18')} />,
      //   onClick: this.onRemoveAllLocalStorage,
      //   tooltip: <FormattedMessage id="app.header.test" key="t" />,
      // },
    ].map((item, i) => {
      let children = (
    ...
    return (
      <div className={this.props.className}>
        ...
        <a className="ant-dropdown-trigger right-menu-button" onClick={this.setRightSideMenuOpen}>
          {
            this.state.rightSideMenuOpen ? <EyeInvisibleOutlined /> : <EyeOutlined />
          }
        </a>
